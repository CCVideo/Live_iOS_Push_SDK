...
//            NSString *userId = [result substringFromIndex:rangeUserId.location + rangeUserId.length];
            NSString *userId = [result substringWithRange:NSMakeRange(rangeUserId.location + rangeUserId.length, 16)];
...
-(UIBarButtonItem *)rightBarPicBtn {
    if(_rightBarPicBtn == nil) {
        ...
        [_rightBarPicBtn setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:FontSize_30],NSFontAttributeName,[UIColor whiteColor],NSForegroundColorAttributeName, nil] forState:UIControlStateNormal];
    }
